...
@section('title', 'Gestión de Productos - Botica AMY')
...
">
    ...
    <!-- Búsqueda -->
    <form method="GET" action="{{ route('productos.index') }}" style="margin-bottom: 20px;">
        <div style="display: flex; gap: 10px;">
            <input 
                type="text" 
                name="search" 
    ...
    @if($productos->count() === 0)
        <p style="text-align: center; color: #7f8c8d;">
            {{ $search ? 'No se encontraron productos con esa búsqueda' : 'No hay productos registrados' }}
        </p>
    @else
        <div style="
            display: grid;
        ...
        ">
            @foreach($productos as $producto)
                ...
                ">
                    <h3 style="margin: 0 0 10px 0; color: #2c3e50;">{{ $producto->nombre_producto }}</h3>
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 10px;">
                        ...
                        <span style="
                            color: {{ $producto->stock <= 10 ? '#e74c3c' : '#2ecc71' }};
                            font-weight: bold;
                        ">
