...
function UsuarioFormModal({ onClose, onSuccess, usuario }) {
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(44,62,80,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
      <form onSubmit={handleSubmit} style={{ background: 'white', borderRadius: 12, boxShadow: '0 6px 16px rgba(0,0,0,0.12)', padding: 24, minWidth: 360, maxWidth: 420, display: 'grid', gap: 12 }}>
        <h3 style={{ margin: 0, color: '#2c3e50' }}>{isEdit ? 'Editar Usuario' : 'Nuevo Usuario'}</h3>
        {error && <div style={{ color: '#e74c3c', background: '#fdecea', padding: 10, borderRadius: 8, textAlign: 'center' }}>{error}</div>}
        {!isEdit && (
          <div>
            <label style={{ display: 'block', marginBottom: 6, color: '#2c3e50' }}>Nombre completo</label>
            <input name="nombre_completo" value={form.nombre_completo} onChange={handleChange} style={{ width: '100%', padding: 10, border: '1px solid #ddd', borderRadius: 6 }} />
          </div>
        )}
        <div>
          <label style={{ display: 'block', marginBottom: 6, color: '#2c3e50' }}>Usuario</label>
          <input name="nombre_usuario" value={form.nombre_usuario} onChange={handleChange} style={{ width: '100%', padding: 10, border: '1px solid #ddd', borderRadius: 6 }} />
          ...
          {!isEdit && form.nombre_completo && (
            <div style={{ marginTop: 6, fontSize: 12, color: '#7f8c8d' }}>
              ...
              {suggestUsername(form.nombre_completo) && (
                <button type="button" onClick={() => setForm((prev) => ({ ...prev, nombre_usuario: suggestUsername(prev.nombre_completo) }))} style={{ marginLeft: 8, padding: '4px 8px', backgroundColor: '#e5e7eb', border: 'none', borderRadius: 6, cursor: 'pointer' }}>Usar</button>
              )}
            </div>
          )}
        </div>
        <div>
          <label style={{ display: 'block', marginBottom: 6, color: '#2c3e50' }}>Contraseña {isEdit ? '(opcional)' : ''}</label>
          <input name="password" type="password" value={form.password} onChange={handleChange} style={{ width: '100%', padding: 10, border: '1px solid #ddd', borderRadius: 6 }} />
          {fieldErrors.password && <span style={{ color: '#e74c3c', fontSize: 13 }}>{fieldErrors.password}</span>}
        </div>
        <div>
          <label style={{ display: 'block', marginBottom: 6, color: '#2c3e50' }}>Rol</label>
          <select name="rol" value={form.rol} onChange={handleChange} style={{ width: '100%', padding: 10, border: '1px solid #ddd', borderRadius: 6 }}>
...
export default function UsuarioAdmin() {
  ...
  return (
    <div style={{ backgroundColor: 'white', borderRadius: 12, boxShadow: '0 6px 16px rgba(0,0,0,0.06)', padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h2 style={{ margin: 0, color: '#2c3e50' }}>Administración de Usuarios</h2>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: 24 }}>
          <p style={{ color: '#7f8c8d' }}>Cargando usuarios...</p>
        </div>
      ) : (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ backgroundColor: '#f7f9fc', color: '#2c3e50' }}>
                <th style={{ textAlign: 'left', padding: 12, borderBottom: '1px solid #eee' }}>Usuario</th>
                <th style={{ textAlign: 'left', padding: 12, borderBottom: '1px solid #eee' }}>Rol</th>
                <th style={{ textAlign: 'center', padding: 12, borderBottom: '1px solid #eee' }}>Estado</th>
                <th style={{ textAlign: 'right', padding: 12, borderBottom: '1px solid #eee' }}>Acciones</th>
              </tr>
            </thead>
            <tbody>
              {usuarios.map((u) => (
                <tr key={u.id_usuario}>
                  <td style={{ padding: 12, borderBottom: '1px solid #f0f0f0' }}>{u.nombre_usuario}</td>
                  <td style={{ padding: 12, borderBottom: '1px solid #f0f0f0' }}>{u.rol === 'admin' ? 'Administrador' : 'Vendedor'}</td>
